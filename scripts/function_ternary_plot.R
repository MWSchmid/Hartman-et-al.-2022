                  ...
                  main = "ternary plot", newpage = TRUE, pop = TRUE, ...)
{
  ...
  # that's original, but seems weird/wrong to me (appears much less specific than I would expect), though it seems correct (simplified formula from https://mathworld.wolfram.com/TernaryDiagram.html)
  xp <- x[, 2] + x[, 3]/2
  yp <- x[, 3] * top
  ...
  grid.points(xp, yp, pch = pch, gp = gpar(col = col), default.units = "snpc",
              size = size, ...)
  #mask <- grepl("grey|gray", col)
  #grid.points(xp[mask], yp[mask], pch = pch[mask], gp = gpar(col = col[mask]), default.units = "snpc",
  #            size = unit(as.numeric(gsub("lines", "", size[mask])), "lines"), ...)
  #grid.points(xp[!mask], yp[!mask], pch = pch[!mask], gp = gpar(col = col[!mask]), default.units = "snpc",
  #            size = unit(as.numeric(gsub("lines", "", size[!mask])), "lines"), ...)
  if (!is.null(id))
