...
if (keepOnlyRootSamples & (!removeRhizobia)) {
  f.print.message("INFO: Removing non-root samples!")
...
if (removeRhizobia) {
  ...
  samplesToUseForRhizobiaFinder <- unique(sampleTab$Sample_id[(sampleTab$Type_sample == "Nodule") & (sampleTab$Species_name != "Lupin")])
  ...
  otusToRemove <- candidatesForRemoval[taxModAll[candidatesForRemoval,"genus"] %in% unique(grep("hizobium", taxModAll$genus, value = TRUE))]
