...
for (cc in names(sigOTUs)) {
  ...
  for (curTaxum in setdiff(colnames(taxMod), "domain")) {
    ...
    write.csv(lfcByTaxum, file.path(eDir, paste0("lfcAveragedBy_", curTaxum, '_', cc, ".csv")), row.names = FALSE)
    ...
    write.csv(enriTest, file.path(eDir, paste0("tax_", curTaxum, '_', cc,"_enrichmentAny.csv")))
    write.csv(enriTestUp, file.path(eDir, paste0("tax_", curTaxum, '_', cc,"_enrichmentUp.csv")))
    write.csv(enriTestDown, file.path(eDir, paste0("tax_", curTaxum, '_', cc,"_enrichmentDown.csv")))
    if (curTaxum %in% c("phylum", "class", "order", "family", "genus", "species")){ # just plot all of them anyway...
      ...
      if (is.null(nrow(toPlot))) {
        f.print.message("skipping", cc, "(", length(sigOTUs$any), ")", curTaxum)
        next
      }
      ...
      f.open.figure(eDir, paste0("heatmap_", curTaxum, '_', cc, "_LFCs.svg"), TRUE, width = 8, height = 7)
      par(oma = c(5, 0, 0, 15))
...
write.csv(allRes, file.path(eDir, "summary_tax_onlySignificant.csv"), row.names = FALSE)
allRes <- subset(allRes, FDR_separate < 0.05)
write.csv(allRes, file.path(eDir, "summary_tax_onlySignificant_FDR.csv"), row.names = FALSE)
