...
save(seqDat, normData, sampleTab, taxMod, taxModAll, noduleOTUs, keepOnlyRootSamples, removeRhizobia, useRarefaction, file = file.path(rDir, "forDownstreamAnalyses.Rdata"))
...
if (length(unique(subSampleTab$Type_sample)) == 1) {
  RDA2 <- rda(subOtus ~ Block + Pot + AMF + Rhizobia+ Species_name, subSampleTab)
} else {
