...
keepOnlyRootSamples <- FALSE # whether to keep only root samples
if ("--onlyRootSamples" %in% myarg) {
  keepOnlyRootSamples <- TRUE
}
removeRhizobia <- FALSE # whether to remove primary symbionts, keeps only root samples
if ("--removeRhizobia" %in% myarg) {
  removeRhizobia <- TRUE
}
useRarefaction <- FALSE # automatically only keeps root samples
...
##########################################################################################
# If rarefaction and 16S, only keep the root samples
if (useRarefaction & (!removeRhizobia)) {
  f.print.message("INFO: REMOVING ALL non-root samples from data with rarefaction!")
...
if (removeRhizobia) {
  ...
  samplesToUseForRhizobiaFinder <- unique(sampleTab$Sample_id[(sampleTab$Type_sample == "Nodule") & (!(sampleTab$Species_name %in% c("Lupin", "Alfalfa", "Medicago", "Lotus")))])
  subSampleTabAll <- subset(sampleTab, Sample_id %in% samplesToUseForRhizobiaFinder)
  otusToRemove <- list()
  candidatesForRemovalList <- list()
  for (curSpecies in unique(subSampleTabAll$Species_name)) {
    subSampleTab <- subset(subSampleTabAll, Species_name == curSpecies)
    subDat <- seqDat[, rownames(subSampleTab)]
    totSum <- rowSums(subDat)
    binSum <- rowSums(subDat>0)
    combi <- (binSum>1)&(totSum>9)
    subDat <- subDat[combi,]
    f.print.message("removed", nrow(seqDat)-nrow(subDat), "OTUs with too little coverage for Rhizobia identification")
    # differential abundance
    formulaString <- "~Type_sample"
    ...
    f.print.message("Found", length(candidatesForRemoval), "candidates for removal in", curSpecies)
    ...
    otusToRemove[[curSpecies]] <- candidatesForRemoval[taxModAll[candidatesForRemoval,"genus"] %in% unique(grep("hizobium", taxModAll$genus, value = TRUE))]
    candidatesForRemovalList[[curSpecies]] <- candidatesForRemoval
  }
  candidatesForRemoval <- Reduce(union, candidatesForRemovalList)
  f.print.message("Found", length(candidatesForRemoval), "candidates for removal")
  otusToRemove <- Reduce(union, otusToRemove)
  ...
  seqDat <- seqDatCopy[OTUsToKeep, rownames(sampleTab)]
  totSum <- rowSums(seqDat)
  binSum <- rowSums(seqDat>0)
  combi <- (binSum>minBinOcc)&(totSum>minTotCou)
  seqDat <- seqDat[combi,]
...
save(seqDat, normData, sampleTab, taxMod, taxModAll, keepOnlyRootSamples, removeRhizobia, useRarefaction, file = file.path(rDir, "forDownstreamAnalyses.Rdata"))
...
if (length(unique(subSampleTab$Type_sample)) == 1) {
  RDA2 <- rda(subOtus ~ Pot + Species_name + AMF + Rhizobia, subSampleTab)
} else {
